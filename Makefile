...
INCDIR = $(CHIBIOS)/os/license \
         ...
         $(HALINC) $(PLATFORMINC) $(BOARDINC) $(STREAMSINC) \
	 $(SHELLINC) $(CHIBIOS)/os/various ./include
...
download_v2: build/ch.elf
	openocd -s $(OCDSCRIPTS)/ -f $(OCDSCRIPTS)/board/st_nucleo_l476rg.cfg  \
	    -c "program build/ch.elf verify reset exit"

openocd_v2: build/ch.elf
	cp gdbinit build/.gdbinit
	openocd -s $(OCDSCRIPTS)/ -f $(OCDSCRIPTS)/board/st_nucleo_l476rg.cfg  \
	    -c "stm32l4x.cpu configure -rtos ChibiOS" \
